...
public class C68_FileReadWriteTest {
	...
	//main 메소드가 throws Exception 한다면 JVM에게 넘긴다. 처리내용은 printStackTrace결과와 동일
	public static void main(String[] args){
		
		String filename = "C:\\Users\\Hwamin Kang\\자바C.txt";
		
		try {
			fileWrite2(filename);
//			filename = "C:\\Users\\Hwamin Kang\\자바0618.txt"; 없는 파일 테스트용
			fileRead(filename);
		} catch (FileNotFoundException e) {
			System.out.println("사용자 실행 오류 : " + e.getMessage());
			System.out.println("파일이 존재하는지 확인하시오.");
		}
		
	}
	
	//Exception을 try~catch로 직접처리
	...
	//throws 키워드는 해당 Exception을 호출한 곳으로 넘긴다.
