...
//테이블마다 필요한 insert, update, delete, select 주요 SQL 동작을 메소드로 작성한 클래스이다. (DAO)
//CustomDao는 TBL_CUSTOM 테이블을 대상으로 하는 SQL 실행 메소드를 포함.
//Singleton 패턴 적용해보기 : 객체를 오직 1개만 생성하는 유형이다.
public class CustomDao { // 자기자신 타입으로 객체를 참조하는 변수
	private static CustomDao dao = new CustomDao(); //자기 자신 타입으로 객체를 참조하는 변수.

	private CustomDao() { // 생성자는 private. 다른 클래스에서 new CustomDao() 못한다.
	}
	...
	public CustomVo getOne (String customId) {
		Connection conn = OracleConnectionUtil.connect();
		String sql ="SELECT * FROM TBL_CUSTOM WHERE CUSTOM_ID = ?"; //0 or 1개의 행
		PreparedStatement pstmt = null;
		ResultSet rs = null;
