...
//vo : value object, 데이터값을 저장하는 목적으로 사용하는 클래스
public class SelectVoTest {

	public static void main(String[] args) {
		String sql ="SELECT * FROM TBL_CUSTOM WHERE CUSTOM_ID = ?"; // 기본키 컬럼으로 조회는 if로 rs.next();
//      String sql ="SELECT * FROM TBL_CUSTOM WHERE age =25";  // 조회 결과가 n개 일 때 while 로 rs.next() 반복
 ...
 * Implement해서 사용 가능해야 한다. Connection, PreparedStatement는 AutoClosable 구현 클래스
 * AutoClosable을 구현한 객체라면 try문이 종료될 때 close() 구문을 호출해준다.
 * SelectWhereTest 클래스의 finally로 직접 close 한 것은 지워.
 */
